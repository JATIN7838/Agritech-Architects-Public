...
class API {
  ...
  String aqi = 'yes';
  String alerts = 'no';
  API({required this.loaction});
  Future<dynamic> getCurrentWeather({required location}) async {
    var uri = Uri.parse('$baseUrl$currentWeather?key=$apiKey&q=$loaction');
  ...
  Future<dynamic> getForecastWeather({required location}) async {
    var uri = Uri.parse(
        '$baseUrl$forecastWeather?key=$apiKey&q=$loaction&days=$days&aqi=$aqi&alerts=$alerts');
    var response = await http.get(uri);
    if (response.statusCode == 200) {
      var data = jsonDecode(response.body);
      return data;
