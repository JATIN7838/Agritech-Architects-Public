...
import 'package:smartagri/facilities/api.dart';
...
class _AssistantState extends State<Assistant> {
  ...
  Future<String> translateText(String text) async {
    try {
      ...
      const clientSecret = 'e0a93b223817485d8e62e181759c3bf3';
      ...
      if (response.statusCode == 200) {
        return jsonDecode(response.body)[0]['translations'][0]['text'];
      } else {
  ...
  Future<String> processRequest(List choices) async {
    ...
    if (!requestBody.contains("gpt-3.5-turbo")) {
      return requestBody;
    }
    final response = await http.post(
      uri,
    ...
    if (choices[0]['message']['content'] != null &&
      ...
      return utf8
          .decode(choices[0]['message']['content'].runes.toList())
          .toLowerCase();
    } else {
  ...
  Future<String> fetchChatResponse(String text) async {
    ...
    var output = jsonDecode(response.body);
    var choices = output['choices'] as List;
    var messageContent = '';
    if (choices[0]['message']['content'] != null &&
        choices[0]['message']['content'] != "") {
      messageContent = utf8
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // add a GPS toggle button here
      // actions: [
      //   const Center(child: Text('GPS')),
      //   Switch(
      //     value: gps,
      //     onChanged: (value) {
      //       setState(() {
